...
\title{Addressing high dimensional multi-objective optimization
  problems by islands with overlapping search space} %TITULO PROVISIONAL
...
All these metrics have been used and described in previously presented works \cite{Deb03distributed,Durillo08masterslave,Hiroyasu07discussion,Dorronsoro13superlinear} in Section \ref{sec:soa}.
...
Two different coevolutionary methods have also been used. In both methods, each individual is splitted in different chunks, and every island performs crossover and mutation in specific chunks of the individuals.  As in the baseline, every
...
\subsubsection{Coevolution with overlapped islands}
...
\caption{Overlapped algorithm: every island $p$ modifies the  $p+1$, $p_{th}$ and $p-1$  parts (in grey) of the individuals.}
...
At a first glance results show that dividing the chromosome produces an improvement in all the quality indicators in all the configurations, with respect to the baseline. Also, results show that all the quality indicators lowers when the number of islands is increased, as the subpopulations are smaller. This is consistent with the claim by Durillo in \cite{Dorronsoro13superlinear}, where cooperative coevolutionary MOEAs work better on bigger populations (more than 100 individuals). With respect to the disjoint and overlapped methods, only when using 8 islands the disjoint method always attain better metrics, and more times with the 2048 chromosome length. This can be explained because only 1/8 of the individual is changed in each island, while in the overlapped, 3/8 of the island may be more similar to the baseline. Therefore, there exist some kind of limit point where one method will be preferable to another, depending the number of islands, population size and length.
