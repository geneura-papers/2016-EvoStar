...
%
\begin{abstract}
High-performance problems with large number of decision variables can take advantage of distributed optimization algorithms, for example, dividing their decision space. This paper compares a distributed multiobjective evolutionary algorithm using two different division methods: one where each island only modifies a disjoint part of the chromosome; and another where some sections of the chromosome are modified by more than one island. Different quality metrics have been used for comparing these methods. Results show that dividing the chromosome decision space achieves better results than the baseline. %Pablo: El abstract sigue así así
...
compared with a baseline version of a distributed NSGA-II \cite{Deb00NSGAII}. Results
...
the early 2000s \cite{Talbi08Parallel} to leverage the capabilities of
...
were used in the experimental setup. However, our approach does not
...
The island model has been executed synchronously, using the ECJ Internal Island Model, in a CentOS 5.4 machine with Intel(R) Xeon(R) CPU E5520 @2.27GHz, 16 GB RAM, with Java Version 1.6.0\_16. As in this paper we only focus in the behaviour of our model in a non-real scenario (the islands share the same memory and processor), the migration time will not be as an important factor as in a real distributed system. This task will be addressed in future works. %Pablo: no sé si decir esto aquí
