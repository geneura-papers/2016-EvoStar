...
\begin{figure}[htb]
	...
	\caption{Execution of the system with the three fitness function. The first column belongs to M (promotion of the monomyth), the second column to A (promotion of the facilitators) and the third column to AD (promotion of the distribution of facilitators). The first row shows the evolution of the best individual, the second one shows the number of occurrences of the monomyth found and the third one the number of occurrences of the facilitators}
	\label{fig:results}
\end{figure}
...
The present paper uses Agent-Based Modeling, Logical Reasoning and Genetic Algorithms to promote the appearance of massive backstories for videogames with archetypes that follow the monomyth guidelines. For this, we model emotion-driven autonomous agents that interact in a virtual world called ColorForm where color pieces compete and collaborate in order to change their colors. The virtual environment generates events using logical predicates that are later used by a PROLOG reasoner to deduce the different archetypes that conforms the monomyth. Finally, a genetic algorithm is used to optimize the configuration of the virtual world. We have compared three different fitnesses in order to guide the evolutionary process.
