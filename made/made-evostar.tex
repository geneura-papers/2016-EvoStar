...
An individual for our GA is a chromosome of 52 genes, where the first five are natural numbers in different ranges and the following forty seven are real values in the range [0-1]. The structure of the chromosome is detailed in figure~\ref{fig:abm_parameters}.

Following A, our implementation of GA is canonical. We use a generational model, with selection by binary tournament and no elitism. Our mutation operator is polynomial and our crossover operator is blx-alpha, both good elections when our genes are coded as numerical values, following B.
