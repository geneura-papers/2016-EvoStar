...
We use a generational model, with selection by binary tournament and no elitism. Our mutation operator is polynomial and our crossover operator is \mbox{blx-$\alpha$}, both good elections when our genes are coded as numerical values. All these decisions are supported by Eiben and Smith in their book \cite{eiben2010whatis}. The number of individuals in the populations is 30 and the termination condition is to reach 30 generations and the maximum time to deduce the predicates from the world's events is 300 seconds. To mitigate the effects of the noisy search space, the fitness of every individual is the average of 15 trials. The range for number of pieces is $[0-15]$ for each shape, hence the final solution can use $[0-45]$ pieces. The size of the world can vary from $8x8$ (64 cells) to $16x16$ (256). The range of virtual days for each simulation is $[2-128]$.  These values have been selected empirically after preliminary tests, and are summarized in Table~\ref{tab:parameters_ga}.
