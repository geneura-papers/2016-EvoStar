...
facts, present in tables \ref{tab:board_events},
\ref{tab:standard_events}, \ref{tab:conditions} and
\ref{tab:strategies})  and those that are inferred using the Reasoner
...
when a condition is fulfilled it generates an event from the table
table~\ref{tab:conditions} and when an action is executed correctly it
generates an event from table~\ref{tab:strategies}. Our implementation
...
Experiments show that the emergence of the monomyth can be improved by searching facilitators, in other words, the predicates that are found in the monomyth, instead of just focusing in the number of monomyth occurrences. Moreover, rewarding the diversity of archetypes leads to better solutions. Results demonstrate that it is possible to make the monomyth emerge and massively generate backstories populated with archetypes that are proved emotionally engaging for the videogame player.
