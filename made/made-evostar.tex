...
generable by the ABM layer, while the gray ones are inferred by the %FERGU: Generable no existe
...
%FERGU: he movido el párrafo de arriba de la sección anterior a esta

The experiments  we set the environment with the following parameters:
%FERGU: Esta frase reescríbela y pon referencia, no pongas :tabla porque 
%a lo mejor se va a otra página (como pasaba en mi versión compilada).
%En LaTeX intenta evitar el :figura :tabla etc. 
%Usa siempre referencias a labels
...
\begin{table}[h!tb]
	...
	\end{center}
\end{table}
%FERGU: IMPORTANTE: no has explicado algunos parámetros, como el número de trials o el tiempo de razonamiento
