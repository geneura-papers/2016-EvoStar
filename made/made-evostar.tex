...
The virtual environment created for this work, the pieces (our characters) can perform different strategies to interact with each other in order to generate events that will conform later backstories. We use the behavior tree of figure~\ref{fig:bt_autogen}, that has condition nodes and action nodes. When a condition is fulfilled it generates an event from the table table~\ref{tab:conditions} and when an action is executed correctly it generates an event from table~\ref{tab:strategies}. Our implementation of behavior tree executes the tree in pre-order until a leaf node is evaluated as 'success'.
