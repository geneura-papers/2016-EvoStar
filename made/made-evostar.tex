...
% Results are coming... FERGU: eso, ponerlos
...
%FERGU: Usas mucho los dos puntos para luego empezar una frase en mayúscula, eso está bien? Preferiría otro conector
...
%FERGU: Lo mismo. dos puntos y mayúscula me suena raro
...
parameterization of the simulation and the output format. Then, in
...
\begin{description}
	\item[{\bf World}]: Our backstories need spatio-temporal
          ...
          %FERGU: homogenizar ColourForms, a veces es ColorForms y otras no usas el resaltado del texto
...
%FERGU: no me gusta nada entrecomillar frases de otros trabajos, sobre todo
%si son tan cortas. Mejor explicalo con tus palabras, en serio
...
Here we contemplate two types of predicates: Those that
%FERGU: el :mayúscula ese
...
but we will focus in the eight archetypes that conform the monomyth:
...
\begin{enumerate}
	...
	\item The \textbf{Threshold Guardians} are the  forces that the hero must overcome.
...
following a dependency graph. The monomytic archetypes are open to interpretations and can be instantiated in many different ways inside a specific story, as many as different films, books, comics or videogames with a hero we could think about. For this reason, we have implemented our own interpretation of the Monomyth archetypes in ColorForms, that are described with other needed concepts as follows: 
%
\begin{figure}[htb]
	\includegraphics[width=\textwidth]{theory.pdf}
	\caption{Predicate dependencies in the proposed logical theory
          of the monomyth: White nodes represent predicates generable
          by the ABM layer and gray predicates (helpers) are inferred
...
\begin{enumerate}
	\item We implement the \textbf{Conflict} as a sequence of events 
	      that succeed
          in a certain moment where two characters take part (the
          winner and the loser). A
          conflict might appear when a piece scares or displaces
          other, or when it takes a color spot that the other
          desires. If two pieces are friends and one of them is displaced by a third one, the two friends have a \textit{Conflict} with it.
	\item In \textit{ColorForms}, a \textbf{Journey} is the time interval 
		  between \textit{Conflicts} with the same characters. 
          The first \textit{Conflict} must be lost by
          one of them (the \textbf{Hero}) and the second one has to be
          lost by the other (the \textbf{Shadow}), who must be `more
          evil' than the \textit{Hero}, that is, have helped less friends. 
	\item A \textbf{Mentor} in \textit{ColorForms} is a character that has high 
		affinity with the \textit{Hero} during the \textit{Journey} and helps them at least once. 
	\item The \textbf{Threshold Guardians} will be defeated by the \textit{Hero} 
		along the \textit{Journey}. In \textit{ColourForms}, they are losers of conflicts against the \textit{Hero} along a \textit{Journey}.
	\item A \textbf{Herald} is a friend of the \textit{Hero} in a \textit{Journey} that has 
		the initiator conflict.
	\item Our interpretation of \textbf{Ally} is a piece that
	      helps the \textit{Hero} in a \textit{Journey}, either
          giving the \textit{Hero} an extra turn to accomplish their goals or
          going along the \textit{Hero} physically, staying aside in the board, along the \textit{Journey}.
	\item The \textbf{Shapeshifter} in ColorForms will play the \textit{Ally} and the
          \textit{Threshold Guardian} archetypes in the same \textit{Journey}. 
	\item The \textbf{Trickster} is a character that accompanies
          the \textit{Hero} along the \textit{Journey} joking and turning tricks. In our
          implementation, to be trickier than other can imply to have
          more joy than them.
\end{enumerate}
...
to work successfully in generative storytelling by Nairat in his work~\cite{nairat2011character}, and later by \cite{garcia2014my} and \cite{garciaortega2015how}.  
...
%FERGU: el :mayúscula. Además, no sé si el dos puntos frase está bien usado como conector (a lo mejor sí, que JJ confirme)
...
We use a generational model, with selection by binary tournament and no elitism. Our mutation operator is polynomial and our crossover operator is \mbox{blx-$\alpha$}, both good elections when our genes are coded as numerical values. All these decisions are supported by Eiben and Smith in their book \cite{eiben2010whatis}.

The number of individuals in the populations is 30 and the termination condition is to reach 40 generations and the maximum time to deduce the predicates from the world's events is 300 seconds. To mitigate the effects of the noisy search space, the fitness of every individual is the average of 15 trials. The range for number of pieces is $[0-15]$ for each shape, hence the final solution can use $[0-45]$ pieces. The size of the world can vary from $8x8$ (64 cells) to $16x16$ (256). The range of virtual days for each simulation is $[2-128]$.  These values have been selected after trials and error in order to have a good dataset for further analysis. These values have been selected empirically after preliminary tests, and are summarized in table~\ref{tab:parameters_ga}.
...
The experiments are executed with the three fitness functions previously defined in section~\ref{sec:ec}: Amount of Monomyth Occurrences Found (M), Amount of Monomyth Occurrences Found ($M$) and Amount of All Ocurrences Distributed by Predicates ($AD$). The results obtained can be seen in table~\ref{fig:results}:
\begin{enumerate}
	\item The predicate monomyth depends directly and indirectly on a huge amount of predicates (world's events, helpers and facilitators, as the dependecy graph in figure~\ref{fig:theory} reflects) hence we cannot search it directly, as seen in the graphs called 'Fitness M' and 'Monomyth in fitness M'. However, if we guide the evolutionary process by taking into account also the facilitator predicates, as A and D do, we can make the whole monomyth emerge with our system.
	\item Although more experiments would clarify it, it seems that the fitness AD gets better solutions than fitness A, since we promote the appearance of new the predicates (and all of them are necessary for the monomyth). In graph 'Monomyth fitness A' we can observe that the GA the monomyth punctually but loses it in favor of new appearances of the ally, as the graph 'facilitarors in fitness A' shows; nevertheless, it does not happen in AD, as shown in 'Monomyth in fitness AD', where the number of monomyths fluctuates but keep a tendency.
	\item Regarding the facilitators found, their rate of appearance is aligned with the real applications of the monomyth: Allies, Guardians and Mentors have much more occurrences tan the rest. The Hero and the Shadow are the leading roles and they must appear only the necessary. Finally Shapeshifters and Tricksters are uncommon characters that depends on predicates that are hard to satisfy. These appearance rates also confirms that our logical model for the monomyth is aligned with the one described by Vogler. However, the fitness AD promotes less difference in the number of appearances than fitness A. As we are not interested in the number of specific archetypes but in the number of monomyths found, we favour the use of AD.         
\end{enumerate}
